...
@var subBtnId = subBtnId!(formId+'-saveDanger');
...
// 							$curmenu.trigger('click');
							...
							if("${searchBtnId}" != undefined && "${searchBtnId}" != ""){
								...
								$curmenu.trigger('click');
								window.location.href="${ctxPath}/#/ajax/artificial/list";  
								/*  $.ajax({  
							    	async: false, 
							        url: "artificial/submit",  
							        data: { "id": id ,state:'2'},  
							        type: "POST",  
							        traditional: true,  
...
//打开指定的标签
	...
	$("#${formId}-reset").click(function(){
        alert("10");
		if("${formId}"=="sysuser-save"){
		  	var id =  $("#jiluuser_id").val();
			layer.closeAll();
			$("#user-edit"+id).trigger('click');
		}else if("${formId}"=="role-save"){
			var id =  $("#jiluuserrole_id").val();
			layer.closeAll();
			$("#role-edit"+id).trigger('click');
		}else{
			document.getElementById("${formId}").reset();
		}
		return false;
