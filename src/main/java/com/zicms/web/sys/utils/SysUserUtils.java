...
/**
    ...
    public static Map<String, SysResource> getUserResources() {
        ...
        Map<String, SysResource> userResources = CacheUtils.get(Constant.CACHE_SYS_RESOURCE, Constant.CACHE_USER_RESOURCE + sysUser.getId());
        if (userResources == null) {
            ...
            CacheUtils.put(Constant.CACHE_SYS_RESOURCE, Constant.CACHE_USER_RESOURCE + sysUser.getId(), userResources);
        }
...
/**
    ...
    public static List<SysResource> getUserMenus() {
        SysUser sysUser = getCacheLoginUser();
        List<SysResource> userMenus = CacheUtils.get(Constant.CACHE_SYS_RESOURCE, Constant.CACHE_USER_MENU + sysUser.getId());
        if (userMenus == null) {
            ...
            CacheUtils.put(Constant.CACHE_SYS_RESOURCE, Constant.CACHE_USER_MENU + sysUser.getId(), userMenus);
        }
        return userMenus;
    }
...
/* public static List<SysRole> getUserRoles() {
        SysUser sysUser = getCacheLoginUser();
        List<SysRole> userRoles = CacheUtils.get(Constant.CACHE_SYS_ROLE, Constant.CACHE_USER_ROLE + sysUser.getId());
        if (userRoles == null) {
            if (sysUser.isAdmin()) {
            ...
            CacheUtils.put(Constant.CACHE_SYS_ROLE, Constant.CACHE_USER_ROLE + sysUser.getId(), userRoles);
        }
        return userRoles;
    }*//*
...
/*public static List<SysOffice> getUserOffice() {
        SysUser sysUser = getCacheLoginUser();
        List<SysOffice> userOffices = CacheUtils.get(Constant.CACHE_SYS_OFFICE, Constant.CACHE_USER_OFFICE + sysUser.getId());
        if (userOffices == null) {
            ...
            CacheUtils.put(Constant.CACHE_SYS_OFFICE, Constant.CACHE_USER_OFFICE + sysUser.getId(), userOffices);
        }
        return userOffices;
    }*//*
...
/*public static List<String> getUserDataScope() {
        SysUser sysUser = getCacheLoginUser();
        List<String> dataScope = CacheUtils.get(Constant.CACHE_SYS_OFFICE, Constant.CACHE_USER_DATASCOPE + sysUser.getId());
        if (dataScope == null) {
            ...
            CacheUtils.put(Constant.CACHE_SYS_OFFICE, Constant.CACHE_USER_DATASCOPE + sysUser.getId(), dataScope);
        }
        return dataScope;
    }*//*
    ...
    public static void clearAllCachedAuthorizationInfo(List<Long> userIds) {
        if (CollectionUtils.isNotEmpty(userIds)) {
            for (Long userId : userIds) {
                boolean evictRes = CacheUtils.remove(Constant.CACHE_SYS_RESOURCE, Constant.CACHE_USER_RESOURCE + userId);
                ...
                boolean evictOffice = CacheUtils.remove(Constant.CACHE_SYS_OFFICE, Constant.CACHE_USER_OFFICE + userId);

                boolean evictScope = CacheUtils.remove(Constant.CACHE_SYS_OFFICE, Constant.CACHE_USER_DATASCOPE + userId);
...
/**
    ...
    public static void cacheLoginUser(SysUser sysUser) {
        CacheUtils.put(Constant.CACHE_SYS_USER, sysUser.getId().toString(), sysUser, getSession().getMaxInactiveInterval());
    }
    ...
    public static SysUser getCacheLoginUser() {
        try {
            if (getSessionLoginUser() != null) {
                return CacheUtils.get(Constant.CACHE_SYS_USER, getSessionLoginUser().getId().toString());
            }
        } catch (Exception e) {
        }
        return null;
